...
if (isset($_POST['add']) && $_POST['add'] !== "") {
    ...
    $selectTag = "SELECT * FROM `tags_tbl` WHERE `tag_name` = '$tag_name'";
    ...
    if ($query_select_run) {
        if (mysqli_num_rows($query_select_run) > 0) {
            ...
            $query_insert = "INSERT INTO `tags_tbl` (`tag_name`, `status`) VALUES ('$tag_name','$status')";
...
elseif (isset($_GET['id'])) {
    $id = $_GET['id'];
    $query_edit = "SELECT * FROM `tags_tbl` WHERE id = $id";
...
elseif (isset($_POST['tag_id']) && $_POST['tag_id'] !== "") {
    ...
    $query_update = "UPDATE `tags_tbl` SET `tag_name` = '$updTagName', updated_at = NOW() WHERE id = $id";
...
elseif (isset($_GET['deleteTags'])) {
    $id = $_GET['deleteTags'];
    $query_delete = "DELETE FROM `tags_tbl` WHERE id = $id";
